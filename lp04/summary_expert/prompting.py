...
def get_page_to_markdown_prompt_template(source_text: str) -> str:
...
def get_page_refine_prompt_template(source_text: Dict[str, Any]) -> str:
    return SystemMessage(
...
pages_combine_prompt_template = """
You are an AI assistant whose goal is to assist in creating detailed summaries of web pages.  You will
be provided with a collection of markdown-formatted summaries (listed below as source_texts), and your task
is to combine the summaries into a single, coherent document.
...
<summary_guidelines>
- You will try to retain as much of the original detail as possible
- You will not add any information that is not present in the source_texts
- You may omit information that is not relevant to the summary
- You may rephrase and restructure the information to increase clarity
- You MUST produce output in a human-readable format following markdown conventions
</summary_guidelines>

The list of source texts is <source_texts>{source_texts}</source_texts>.
"""

def get_pages_combine_prompt_template(source_texts: str) -> str:
    return SystemMessage(
        content=pages_combine_prompt_template.format(
